...
//仿照netcam_update_mail_style函数编写的将http上传的数据包提取并存储升级数据的函数//
...
//********OCR 算法更新**********//
static int ai_upgrade_read_cb(HTTP_OPS* ops, void* arg)
{
        int data = 1;
        char retData[12];
        sprintf(retData,"%d",data);
...
static int ai_upgrade_read(HTTP_OPS *ops, void *arg)
{
    char *data = NULL;
    char *tag;
    int bodyLen;
          ...
          if (netcam_update_http_style(data, bodyLen, NULL) == 0)
            {
                ...
                if (rename("/sharefs/ocr/bin/eac_ocr", "/sharefs/ocr/bin/eac_ocr_bak") != 0)
                {
                    PRINT_ERR("Failed to rename /sharefs/ocr/bin/eac_ocr to /sharefs/ocr/bin/eac_ocr_bak\n");
                }

                int file_fd = open("/sharefs/ocr/bin/eac_ocr", O_WRONLY | O_CREAT | O_TRUNC, 0644);
                if (file_fd < 0)
                {
                    PRINT_ERR("Failed to open /sharefs/ocr/bin/eac_ocr for writing\n");
                    goto ERROR_EXIT;
                }
                ...
                if (written_len != recvLen)
                {
                    PRINT_ERR("Failed to write all data to /sharefs/ocr/bin/eac_ocr\n");
                    close(file_fd);
                    goto ERROR_EXIT;
                }

                close(file_fd);
                printf("Data successfully saved to /sharefs/ocr/bin/eac_ocr\n");
